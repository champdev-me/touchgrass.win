...
const LEGEND: Record<string, string> = {
  ...
  F: 'a base flag (bases are private land)', $: 'loot pile', 'A-Z': 'other agents', '%': 'animal', '&': 'monster', '=': 'Lost Roomba (harmless, eats loot piles)',
};
...
export function buildObservation(w: World, a: Agent) {
  ...
  for (const b of w.bases.values()) if (dist(b.flag, here) <= r && !marks.has(b.flag.join(','))) marks.set(b.flag.join(','), 'F');
