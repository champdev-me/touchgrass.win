...
test('options and effects: sprint, steady, conserve, overtake on a clear leg', () => {
  ...
  assert.deepEqual([r('a').distance, r('a').stamina], [24, 12]);
  assert.deepEqual([r('b').distance, r('b').stamina], [20, 14]);
  assert.deepEqual([r('c').distance, r('c').stamina], [16, 15]); // capped at 15
...
test('leg events: mud, tailwind, hill, and the home stretch on the last leg', () => {
  ...
  assert.equal(s.runners.get('a')!.stamina, 10); // sprint 3 + mud 2
...
test('three laps of the oval: straight, hurdle, turn, hurdle, turn, then the home stretch', () => {
  const s = H.start(['a', 'b', 'c', 'd'], fixed(0.1));
  const seen: string[] = [];
  while (!H.finished(s)) {
    seen.push(s.event);
  ...
  assert.deepEqual(seen, ['clear', 'hurdle', 'turn', 'hurdle', 'turn', 'tailwind', 'hurdle', 'turn', 'hurdle', 'turn', 'tailwind', 'hurdle', 'turn', 'hurdle', 'home_stretch']);
});
...
test('hurdles: jump appears and always clears; sprints clip half the time and lose 8', () => {
  ...
  const r = (id: string) => s.runners.get(id)!;
  assert.deepEqual([r('a').distance, r('a').stamina], [17, 13]); // jump 18 - 1
...
test('fifteen legs; winner by distance, then stamina, then lot', () => {
  const s = H.start(['a', 'b', 'c', 'd'], fixed(0.1));
  for (let i = 0; i < 15; i++) H.resolve(s, new Map([['a', 3], ['b', 2], ['c', 2], ['d', 2]]), noLuck);
